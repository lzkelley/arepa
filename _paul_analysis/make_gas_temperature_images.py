...
#                 'explicit_feedback_512', \
    ...
    for snapnum in [10]:        #range(n_snaps):        #range(n_snaps):        #range(13):
        ...
        for groupnr in range(1):        #range(np.min([10, len(cat.GroupLenType[:,0])] ) ):
            if cat.GroupLenType[groupnr,4] > 500:
                center = cat.GroupPos[groupnr,:]

                fig_,(ax1_,ax2_,ax3_)=plt.subplots( 1, 3, figsize=(3,1) )
                image,massmap = gas_images.gas_image( '../'+run_base+'/output/', snapnum, center=center, \
...
#                image,massmap = gas_images.gas_image( '../'+run_base+'/output/', snapnum, center=center, \
#                                                        xrange=[-0.01,0.01], yrange=[-0.01,0.01], zrange=[-0.01, 0.01], \
#                                                        pixels=npixels , cosmo_wrap=True, massmap=False, projaxis=1, \
#                                                maxden = 2e3, dynrange = 1e3, 
#                                                unit_length_mpc=True)
...
#                image,massmap = gas_images.gas_image( '../'+run_base+'/output/', snapnum, center=center, \
#                                                        xrange=[-0.01,0.01], yrange=[-0.01,0.01], zrange=[-0.01, 0.01], \
#                                                        pixels=npixels , cosmo_wrap=True, massmap=False, projaxis=2, \
#                                                maxden = 2e3, dynrange = 1e3, 
#                                                unit_length_mpc=True)
