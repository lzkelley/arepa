...
class CustomLightSource(object):
    ...
    def shade(self,data,cmap,vmin=None,vmax=None,limit_elevation=False):
        ...
        if (vmin is not None) or (vmax is not None):
            ...
            limitschanged = False
        if vmin is None:
            vmin = np.percentile(data, 5.0) 	#min(data)
        if vmax is None:
            vmax = np.percentile(data, 95.0) 	#np.max(data)

	#print data-vmin
	#print np.float(vmax-vmin)
	#print (data-vmin) / (np.float(vmax-vmin) )
	#print cmap( (data-vmin) / (np.float(vmax-vmin) ) )
        if (np.float(vmax-vmin) ) > 0:
