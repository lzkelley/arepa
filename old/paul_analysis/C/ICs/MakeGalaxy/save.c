...
#define BLKLEN fwrite(&blklen, sizeof(blklen), 1, fd);
  ...
  for(i = 1; i <= N_GAS; i++)
    {
      xyz[0] = xp_gas[i] + SPATIALOFFSET;
      xyz[1] = yp_gas[i] + SPATIALOFFSET;
      xyz[2] = zp_gas[i] + SPATIALOFFSET;
      fwrite(xyz, sizeof(float), 3, fd);
    }
  for(i = 1; i <= N_HALO; i++)
    {
      xyz[0] = xp_halo[i] + SPATIALOFFSET;
      xyz[1] = yp_halo[i] + SPATIALOFFSET;
      xyz[2] = zp_halo[i] + SPATIALOFFSET;
      fwrite(xyz, sizeof(float), 3, fd);
    }
  for(i = 1; i <= N_DISK; i++)
    {
      xyz[0] = xp_disk[i] + SPATIALOFFSET;
      xyz[1] = yp_disk[i] + SPATIALOFFSET;
      xyz[2] = zp_disk[i] + SPATIALOFFSET;
      fwrite(xyz, sizeof(float), 3, fd);
    }
  for(i = 1; i <= N_BULGE; i++)
    {
      xyz[0] = xp_bulge[i] + SPATIALOFFSET;
      xyz[1] = yp_bulge[i] + SPATIALOFFSET;
      xyz[2] = zp_bulge[i] + SPATIALOFFSET;
      fwrite(xyz, sizeof(float), 3, fd);
    }

  for(i = 1; i <= N_BLACKHOLE; i++)
    {
      xyz[0] = xyz[1] = xyz[2] = SPATIALOFFSET;
      fwrite(xyz, sizeof(float), 3, fd);
    }
