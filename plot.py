...
def plot_proj_1d(path, scatter_param='Density', hist_param='Masses', snaps=None, hist_dens=True,
    ...
    for snap_num in tqdm.tqdm(snaps):
        fig = lysis.plot.plot_proj_1d_snap(
        ...
        _fname = lysis.save_fig(fig, fname, path, snap_num=snap_num, verbose=False)
        if _fname is None:
            print("BAD!")
            print(_fname, fname)
            raise RuntimeError()
...
def plot_proj_1d_snap(path, snap_num, scatter_param='Density', hist_param='Masses',
    ...
    keys, head, pars = lysis.readio.snapshot(path, snap_num=snap_num, header=True)
    lysis.plot.draw_snap_time(ax, head, num=snap_num)
...
def make_movie(path, fname, output, framerate=10):
    ...
    if not isinstance(fname, list):
        fname = lysis.fname_append_snap(fname, '*')
        file_list = sorted(glob.glob(fname))
        if len(file_list) == 0:
            fname = lysis.fname_append_snap(os.path.join(path, fname), '*')
            file_list = sorted(glob.glob(fname))
    else:
        file_list = fname
