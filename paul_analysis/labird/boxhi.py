...
#        mass   = mass[   :10000   ]
    ...
    def _calc_cddf(self,NHI_table, minN=17, maxM=None,minM=None, metallicity_cut=-1 ):
	...
	if metallicity_cut >= 0:
	    try:
	        z_grids = self.sub_nZ_grid
	    except:
		self.load_nz_grid()
		z_grids = self.sub_nZ_grid
	else:
	    z_grids = np.ones_like( grids )
        ...
        if maxM != None:
            ...
            ind = np.where( (grids >= minN) & (z_grids > metallicity_cut) )
            tot_f_N = np.histogram(np.ravel(grids[ind]),np.log10(NHI_table))[0]
